...
class TelaProdutosDaCategoria extends StatelessWidget {
  //esse documento ira passar a qual categoria o produto pertence
  final DocumentSnapshot snapshot;
  TelaProdutosDaCategoria(this.snapshot);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
          ...
          onPressed: () {
            String categoria = this.snapshot.documentID;
            Navigator.of(context).push(
                MaterialPageRoute(builder: (context) => TelaCRUDProduto(snapshot: this.snapshot,categoria: categoria,))
            );
          }
      ),
            ...
            if (!snapshot.hasData)
              ...
              return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _construirListaProdutos(context, produto, snapshot.data.documents[index]);
                  });
          }),
    );
  }

  Widget _construirListaProdutos(contexto, Produto p, DocumentSnapshot snapshot){
    return InkWell(
      ...
      onTap: (){
        Navigator.of(contexto).push(MaterialPageRoute(builder: (contexto)=>TelaCRUDProduto(produto: p, snapshot: snapshot, categoria: p.categoria,)));
      },
    );
  }
}
